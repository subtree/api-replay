...
// First, update the type definitions at the top of the file
type MockFetch = typeof globalThis.fetch;

// Remove the PartialFetch type as it's no longer needed
...
describe('Request Deduplication During Recording', () => {
  ...
  test('prevents duplicate remote calls with timestamp exclusion', async () => {
    ...
    const mockFetch: MockFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      httpCallCount++;
    ...
    try {
      ...
      const response3 = await fetch(
        'https://jsonplaceholder.typicode.com/posts/1?timestamp=5555555555&nonce=newvalue',
        {
